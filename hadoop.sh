...
tar xzf $HOME/hadoop-3.3.6.tar.gz
...
sudo chown -R $USER:$USER $HOME/dfsdata/
sudo chown -R $USER:$USER $HOME/dfsdata/datanode/
sudo chown -R $USER:$USER $HOME/dfsdata/namenode/
...
sudo mv $HADOOP_HOME/etc/hadoop/tmp-yarn-site.xml $HADOOP_HOME/etc/hadoop/tmp-yarn-site.xml
...
echo "---------------------"
echo "Run ./start-all.sh to start all hadoop processes"
echo "--------------------"
